...
# Bild vom Zug laden
gray_img = cv2.imread("data/canny1.png",  cv2.IMREAD_GRAYSCALE)
...
tmask = cv2.imread("data/sktemplate2mask.png", cv2.IMREAD_GRAYSCALE)
...
print(result.shape)
print(result.max)
cv2.imshow('matchmap', result)

test =  cv2.minMaxLoc(result)



loc = np.where(result >= np.nanmax(result))
...
# test
cv2.imshow('Maske', tmask)
cv2.imshow('zug', img)
cv2.imshow('zug ROI', roi)
