...
def findGrid(imageLeft, imageRight, verbose=False):
    """
    ...
    if imageLeft.ndim > 2:
        imgL = cv2.cvtColor(imageLeft, cv2.COLOR_RGB2GRAY)
    else:
        imgL = imageLeft.copy()

    if imageLeft.ndim > 2:
        imgR = cv2.cvtColor(imageRight, cv2.COLOR_RGB2GRAY)
    else:
    ...
    maxval = min(freqHitsSmoothL.max(), freqHitsSmoothR.max())
    ...
    # Prüfen, ob die Gitterposition gültig ist. Ungültig, wenn Peak zu tief (Erkennungsproblem oder kein Gitter im Bild)
    # oder Gitter zu weit am Bildrand. Grenze für Bildrand: Wenn nicht alle 4 Gitterschrauben sichtbar, ist es zu weit
    # am Rand. Die Gitterposition wäre in diesem Fall nutzlos, da sie ausschliesslich verwendet wird als Referenz zur
    # Suche der vier Gitterschrauben.

    if maxval < 55000:
        return gitterL, gitterR, False

    # Gültiger Pixel Koordinatenbereich (y auf dem Bird's eye view prüfen reicht aus)
    if 850 <= gitterposL <= 3400 and 850 <= gitterposR <= 3400:
        return gitterL, gitterR, True
    else:
        return gitterL, gitterR, False
...
if __name__ == '__main__':
    ...
    #Gitter suchen
    gitterL, gitterR, isValid = findGrid(sbbL_gray, sbbR_gray, verbose=True)
